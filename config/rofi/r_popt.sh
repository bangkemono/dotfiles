...
# confirm
confirmation() {
    echo -e "Yes\nNo"| $rofi_start -font "DinaRemasterII 12" -no-fixed-num-lines -dmenu -p "Are you Sure? "
}

options="Shutdown\nReboot\nLock\nLogout"

chosen="$(echo -e "$options" | $rofi_start -font "DinaRemasterII 12" -p "Power Options" -dmenu -selected-row 2 -lines 4 padding)"
case $chosen in
    Shutdown)
  ans=$(confirmation &)
